...
import { AI_DAILY_LIMIT, AI_LANGUAGE, CampaignIdeasSchema, cleanDrafts, localNow, visitPattern } from "../../../lib/ai-context";
...
const SYSTEM = `Eres el asistente de marketing de Smart Loyalty, una plataforma para negocios pequeños donde los clientes vuelven (restaurantes, cafés, barberías, salones de belleza, spas, gimnasios, tiendas y más). Escribes notificaciones push que el dueño envía a los clientes de su negocio.

Cómo escribir:
- En el idioma que se indica al final, cercano y con el tono del negocio (guíate por su tipo y su descripción).
...
sendAt: fecha y hora local del negocio en formato YYYY-MM-DDTHH:mm, siempre después de "ahora". Apóyate en las horas y días con más visitas: para llenar un momento flojo, envía un par de horas antes. Usa null si conviene enviarla ya.
...
export async function POST(req: NextRequest) {
  ...
  const context = {
    negocio: {
      nombre: company.name ?? "",
      tipo: messages.es.niches.types[cleanBusinessType(company.businessType)],
  ...
  try {
    const ideas = await askAi(
      CampaignIdeasSchema,
      SYSTEM,
      `<datos>\n${JSON.stringify(context, null, 1)}\n</datos>\n\n<objetivo>\n${goal}\n</objetivo>\n\nEscribe name, title, body, coupon.title y why en ${AI_LANGUAGE[companyLocale(company)]}.`
    );
